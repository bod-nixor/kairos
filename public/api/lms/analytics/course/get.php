...
$sections = $pdo->prepare(
    'SELECT s.section_id, s.title,
            ROUND((COUNT(DISTINCT c.completion_id)/NULLIF(COUNT(DISTINCT l.lesson_id)*NULLIF((SELECT COUNT(*) FROM student_courses WHERE course_id=:course_id),0),0))*100,2) AS completion_percent
...
$assignment = $pdo->prepare('SELECT ROUND((COUNT(DISTINCT s.student_user_id)/NULLIF((SELECT COUNT(*) FROM student_courses WHERE course_id=:course_id),0))*100,2) AS submission_rate, ROUND((SUM(CASE WHEN s.is_late=1 THEN 1 ELSE 0 END)/NULLIF(COUNT(*),0))*100,2) AS late_percent FROM lms_submissions s WHERE s.course_id=:course_id');
$assignment->execute([':course_id' => $courseId]);
...
$missing = $pdo->prepare('SELECT q.question_id, q.prompt, COUNT(*) AS misses FROM lms_assessment_responses r JOIN lms_questions q ON q.question_id=r.question_id WHERE q.assessment_id IN (SELECT assessment_id FROM lms_assessments WHERE course_id=:course_id) AND r.score < r.max_score GROUP BY q.question_id, q.prompt ORDER BY misses DESC LIMIT 5');
...
lms_ok([
    ...
    'assignment_stats' => $assignment->fetch(),
